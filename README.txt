Name: Tom Scharfenort

In der .env bei springboot sind zwei Keys (Für JWT und Aszure). Falls GitHub oder so etwas nicht übernommen hat und die fehlen am besten bei mir kurz melden dann schicke ich euch diese
und man kann sie in der properties manuell einfügen. Sollte aber ohne Probleme klappen
...
Ebenso läuft dies aber schon über JWT (Wie bei Luis), so kann man nun auch seine eigene Anzeige wieder löschen.
-> Die Anzeigen können in einem seperaten Fenster angezeigt werden mit den jeweiligen Bilder und Videos. (Sollte ausgelagert werden in eine eigene Komponente -> Bzw zur Startseite)
...
Backend:
	...
	-> Hatte damit Probleme. In application properties kann ein Key übergegeben wodurch den dann die SQLite Datei "encrypted" manuell nicht mehr öffnen lässt.
...
-> Backend starten mit "mvnw spring-boot:run -DskipTests"
	...
	-> Evtl die Webseite kurz aufrufen und Zertifikat erlauben
-> Frontend starten
...
Integration in die Gesamtsoftware Vinted und Schnittstellen zu anderen Komponenten:
...
-> Meine Azure Cloud funktioniert 30 Tage auf jeden Fall Problemlos wie man dann auch in der Präsentation sehen wird oder ihr selber bei euch zu Hause.
-> Danach habe ich eine 12 Monatige kostenlose Version jedoch mit begrenzten Ressourcen, evtl könnte es da zu Problemen dann kommen, das es funktioniert seht ihr aber auf jeden Fall.
